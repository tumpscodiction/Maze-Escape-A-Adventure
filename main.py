...
pygame.display.set_caption("Maze Escape: A* Adventure")

#Colors(RGB)
BACKGROUND = (51, 255, 255)  # Cyan
PATH_CLOSE = (51, 0, 255)  # Light Blue
PATH_OPEN = (0, 0, 153)  # Dark Blue
GRID = (0, 0, 0)  # Black
WALL = (0, 0, 0)  # Black
START = (255, 0, 102)  # Red
END = (76,187,23)  # Green
PATH = (255, 51, 51)  # Orange
...
def h(p1, p2):
    x1, y1 = p1 # splitting values from a tuple
    x2, y2 = p2
    return abs(x1 - x2) + abs(y1 - y2)

def reconstruct_path(node_path, current, draw, counter_start):
    pygame.display.set_caption("Maze Escape ( Constructing Path... )")
    path_count = 0
...
def algorithm(draw, grid, start, end, counter_start):
    ...
    pygame.display.set_caption("Maze Escape ( Unable To Find The Target ! )")
    return False
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                ...
                if not Start and not End:
                    pygame.display.set_caption("Maze Escape ( Set Start & End Position ! )")

                if e.key == pygame.K_SPACE and Start and End:

                    counter_start = timer()
                    pygame.display.set_caption("Maze Escape ( Searching... )")
                    for row in grid:
                        for spot in row:
                ...
                if e.key == pygame.K_c:
                    Start = None
                    End = None
                    pygame.display.set_caption("Maze Escape : ( Using A* Algorithm )")
                    grid = make_grid(ROWS, width)
    pygame.quit()
